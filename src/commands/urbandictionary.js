  ...
  fn: function(msg, suffix) {
    if (!suffix) {
      msg.channel.createMessage(
        `<@${msg.author.id}>, Yes, let's just look up absolutely nothing.`
      );
    } else {
      request
          ...
          if (!err && res.status === 200) {
            ...
            if (uD.result_type !== 'no_results') {
              ...
              msg.channel.createMessage(
                `<@${
                  msg.author.id
                }>, ${suffix}: This word is so screwed up, even Urban Dictionary doesn't have it in its database`
              );
            }
          } else {
            global.logger.error(
              `Got an error: ${err}, status code: ${res.status}`
            );
          }
        });
    }
